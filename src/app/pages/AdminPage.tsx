          ...
          {bookings.map((b) => (
            <div key={b.id} className="p-3 border rounded flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-slate-500">{b.phone} • {b.guests} guests • {new Date(b.booked_at).toLocaleString()}</div>
                <span className={`text-xs px-2 py-0.5 rounded ${b.status === 'accepted' ? 'bg-green-100 text-green-700' : b.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  {b.status}
                </span>
              </div>
              <div className="flex gap-2">
                <button onClick={() => updateBooking(b.id, 'accepted')} className="px-3 py-1 bg-green-600 text-white rounded text-sm">Accept</button>
                <button onClick={() => updateBooking(b.id, 'rejected')} className="px-3 py-1 bg-red-600 text-white rounded text-sm">Reject</button>
              </div>
            </div>
          ))}
        </div>
